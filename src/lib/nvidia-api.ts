...
const NVIDIA_MODEL = 'minimaxai/minimax-m2.1';
...
export async function generateWritingQuestions(difficulty = 'Band 6'): Promise<{ task1: WritingTask; task2: WritingTask }> {
  ...
  try {
    parsed = JSON.parse(content) as NvidiaChatResponse;
  } catch {
    throw new Error('Failed to parse Nvidia response as JSON');
  }

  const task1 = ensureValidTask(parsed.task1?.prompt, 'task1', parsed.task1?.chartType, parsed.task1?.chartData);
